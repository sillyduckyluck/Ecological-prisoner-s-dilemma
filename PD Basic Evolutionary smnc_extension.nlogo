patches-own [
  ...
  policy           ;;The patch's long-term game plan. (tit for tat, etc.)
]

to setup
  clear-all
  ask patches [
    let policy_decision random 4

    ifelse policy_decision = 0
    [set policy "tit"]
    [ifelse policy_decision = 1
      [set policy "coop"]
      [ifelse policy_decision = 2
        [set policy "attk"]
        [set policy "rndm"]]]
...
to select-strategy  ;; patch procedure
  ...
  [set cooperate? true]
  [ifelse policy = "tit"
    [ifelse neighbor-defection-sum > 4[set cooperate? true][set cooperate? false]]
...
to select-policy
  set policy [policy] of max-one-of neighbors [score]
end

to-report neighbor-defection-sum
  report count neighbors with [cooperate? = true]
end

to randomize-cooperate?
  ifelse random 2 = 1
  [set cooperate? true]
  [set cooperate? false]
end

to establish-color-policy-basis
  ifelse policy = "rndm"
  [set pcolor blue
  set color-class 1]
